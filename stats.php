        ...
        if (isset($_SESSION['id'])) {
            ...
            if ($stmt->rowCount() > 0) {
                while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    echo "<h3> {$row['name']} &emsp; </h3><h4>Popularity: {$row['times_clicked']}</h4> &nbsp; Address: {$row['address']}, Hours: {$row['hours']}, Fee: {$row['fee']}, Status: {$row['status']} <br> &nbsp; Description: {$row['description']}";
                    echo "</div>";
                    echo "</a>";
                }
            } else {
